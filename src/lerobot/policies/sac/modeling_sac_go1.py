...
from lerobot.policies.sac.processor_sac_go1 import GO1ObservationPreprocessor
    ...
    def compute_loss_critic(
        ...
        with torch.no_grad():
            ...
            if self.config.use_backup_entropy:
                min_q = min_q - (self.temperature * next_log_probs[:,0])
...
class MaskedAttentionPooler(nn.Module):
    """Project sequence features to a fixed embedding using learned queries."""

    def __init__(self, embed_dim: int, num_heads: int, num_queries: int = 1, dtype: torch.dtype = torch.bfloat16):
        super().__init__()
    ...
    def forward(self, key: Tensor, value: Tensor, mask: Tensor | None = None) -> Tensor:
        """Returns tensor of shape (batch_size, num_queries * embed_dim)."""
        batch_size = key.size(0)
        query = self.query.unsqueeze(0).expand(batch_size, -1, -1).to(dtype=key.dtype, device=key.device)

        key_padding_mask = None
        if mask is not None:
            mask = mask.reshape(batch_size, -1)
            key_padding_mask = ~mask.to(dtype=torch.bool, device=key.device)

        pooled, _ = self.attention(query, key, value, key_padding_mask=key_padding_mask)
...
class SACObservationEncoder(nn.Module):
    ...
    and pools them via a learnable masked attention module to create a compact
    representation for the critic head. The design reduces the high-dimensional
    key-value cache to a manageable size while preserving the most relevant
    information from the VLM's final layer.
    
    The embedding dimension equals n_queries * hidden_size, where hidden_size is the
    transformer width and n_queries is the number of learned pooling queries.
    """

    def __init__(self, config: SACGO1Config, tokenizer, vla_model: GO1Model) -> None:
        super().__init__()
        self.config = config
        self.tokenizer = tokenizer
        self.vla_model = vla_model
        self.hidden_size = self.vla_model.language_model.config.hidden_size
        self.num_attn_heads = self.vla_model.language_model.config.num_attention_heads
        self.critic_pool_num_queries = getattr(self.config, "critic_pool_num_queries", 1)
        self.attention_pooler = MaskedAttentionPooler(
            embed_dim=self.hidden_size // 2,
            num_heads=self.num_attn_heads,
            num_queries=self.critic_pool_num_queries,
            dtype=self.vla_model.dtype,
        )
        self._compute_output_dim()
        self.mlp_head = MLP(
            input_dim=self._out_dim // 2,
            hidden_dims=[self._out_dim, self._out_dim],
            activate_final=True,
    ...
    def _compute_output_dim(self) -> None:
        # Masked attention pooling outputs (num_queries * hidden_size)
        self._out_dim = self.critic_pool_num_queries * self.hidden_size
    ...
    def get_cached_image_features(self, obs: dict[str, Tensor]) -> dict[str, Tensor]:
        ...
        return {"vlm_outputs": vlm_outputs, "attention_mask": obs["attention_mask"]}
    ...
    def _encode_images(self, cache: dict[str, Tensor], detach: bool) -> Tensor:
        ...
        pooled = self._masked_attention_pool(last_key, last_value, attention_mask)
        vlm_features = self.mlp_head(pooled.to(dtype=self.mlp_head.net[0].weight.dtype))
        if detach:
            vlm_features = vlm_features.detach()

        return {"vlm_outputs": vlm_outputs, "vlm_features": vlm_features, "attention_mask": attention_mask}

    def _masked_attention_pool(self, key: Tensor, value: Tensor, attention_mask: Tensor | None) -> Tensor:
        """Convert (B, heads, seq, head_dim) tensors into a pooled embedding."""
        batch_size, num_heads, seq_len, head_dim = key.shape
        key_seq = key.permute(0, 2, 1, 3).reshape(batch_size, seq_len, num_heads * head_dim)
        value_seq = value.permute(0, 2, 1, 3).reshape(batch_size, seq_len, num_heads * head_dim)
        pooled = self.attention_pooler(key_seq, value_seq, attention_mask)
        return pooled
...
class CriticEnsemble(nn.Module):
    ...
    def forward(
        ...
        if len(actions.shape) == 3:
            inputs = torch.cat([obs_enc['vlm_features'], actions[:,0,:]], dim=-1)
        else:
            inputs = torch.cat([obs_enc['vlm_features'], actions], dim=-1)
...
class DiscreteCritic(nn.Module):
    ...
    def forward(
        ...
        observations = {k: v.to(device) for k, v in observations.items()}
        obs_enc = self.encoder(observations, cache=observation_features)
        return self.output_layer(self.net(obs_enc['vlm_features']))
...
class Policy(nn.Module):
    ...
    def forward(
        ...
        vlm_key_values = obs_enc['vlm_outputs'].past_key_values  # kv_cache(multi_head) of each decoder layer
        ...
        timestep_tokens = self.network.time_embedder(timesteps)  # (B, 1, C)
        freq_tokens = self.network.freq_embedder(ctrl_freqs)  # (B, 1, C)
        noisy_action = self.network.noise_scheduler.add_noise(dummy_action, noise, timesteps)  # (B, H, C)
