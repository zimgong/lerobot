...
class GO1ObservationPreprocessor:
    ...
    def prepare_batch(self, observations: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        # with ThreadPoolExecutor(max_workers=min(batch_size, os.cpu_count() // 2)) as executor:
        #     futures = {
        #         executor.submit(
        #             self._prepare_single,
        #             {k: v[b : b + 1] for k, v in observations.items()}
        #         ): b
        #         for b in range(batch_size)
        #     }
        #     for future in as_completed(futures):
        #         batch_idx = futures[future]
        #         features[batch_idx] = future.result()
    ...
    def prepare_single(self, observation: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        """Prepare multimodal inputs for a single (batched) observation."""
        if not observation:
            raise ValueError("Observation dictionary is empty.")
        # Ensure tensors include batch dimension
        single = {k: (v if v.dim() > 0 and v.shape[0] == 1 else v.unsqueeze(0)) for k, v in observation.items()}
        return self._prepare_single(single)
